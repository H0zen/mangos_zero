...
struct boss_arugal : public CreatureScript
{
    ...
    struct boss_arugalAI : public ScriptedAI
    {
                ...
                if (m_uiSpeechTimer < uiDiff)
                {
                    switch (m_uiSpeechStep)
                    {
                        ...
                        case 5:
                        {
                            ...
                            for (uint8 i = 0; i < 4; ++i)
                            {
                                pVoidwalker = SummonCreature(*m_creature, NPC_VOIDWALKER, VWSpawns[i].fX,
        ...
        {
            return (m_creature->getVictim() ? m_creature->Where().DistanceTo(m_creature->getVictim()->Where(), false) : 999.9f);
        }
