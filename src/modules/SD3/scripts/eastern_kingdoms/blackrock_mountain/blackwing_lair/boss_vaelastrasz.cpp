        ...
        void UpdateAI(const uint32 uiDiff) override
        {
            if (m_uiIntroTimer)
            {
                if (m_uiIntroTimer <= uiDiff)
                {
                    ...
                    {
                        case 0:
                            SummonCreature(*m_creature, NPC_LORD_VICTOR_NEFARIUS, aNefariusSpawnLoc[0], aNefariusSpawnLoc[1], aNefariusSpawnLoc[2], aNefariusSpawnLoc[3], TEMPSPAWN_TIMED_DESPAWN, 25000);
                            m_uiIntroTimer = 1000;
                            break;
                        case 1:
