    ...
    : m_announce(true), m_moderate(false), m_name(name), m_flags(0), m_channelId(0)
{
    ...
    if (ch)
    {
        ...
        if (ch->Flags & CHANNEL_DBC_FLAG_CITY_ONLY2)
        {
            m_flags |= CHANNEL_FLAG_CITY;
        }

        if (ch->Flags & CHANNEL_DBC_FLAG_LFG)
        {
...
void Channel::Leave(Player* player, bool send)
{
    ...
    if (changeowner)
    {
        ObjectGuid newowner = !m_players.empty() ? m_players.begin()->second.player : 0;
        SetOwner(newowner);
    }
}
...
void Channel::KickOrBan(Player* player, const char* targetName, bool ban)
{
    ...
    if (changeowner)
    {
        ObjectGuid newowner = !m_players.empty() ? guid : 0;
        SetOwner(newowner);
    }
}
...
void Channel::List(Player* player)
{
    ...
    for (PlayerList::const_iterator i = m_players.begin(); i != m_players.end(); ++i)
    {
        ...
        {
            data << static_cast<ObjectGuid>(i->first);
            data << uint8(i->second.flags);
            ++count;
        }
    }
...
void Channel::Say(Player* player, const char* text, uint32 lang)
{
    ...
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, text, Language(lang), player->GetChatTag(), guid, player->GetName(), 0, "", m_name.c_str(), player->GetHonorRankInfo().rank);
    SendToAll(&data, !m_players[guid].IsModerator() ? guid : 0);
}
...
void Channel::Voice(ObjectGuid , ObjectGuid )
{
}

void Channel::DeVoice(ObjectGuid , ObjectGuid )
{
}
...
void Channel::MakePlayerKicked(WorldPacket* data, ObjectGuid target, ObjectGuid source)
{
    ...
    *data << static_cast<ObjectGuid>(target);
    *data << static_cast<ObjectGuid>(source);
}
...
void Channel::JoinNotify(ObjectGuid )
{

}

void Channel::LeaveNotify(ObjectGuid )
{
