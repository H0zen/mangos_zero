...
template<class T, class CONTAINER>
    inline void Cell::Visit(const CellPair& standing_cell, TypeContainerVisitor<T, CONTAINER> &visitor, Map& m, const Occupant& obj, float radius) const
{
...
template<class T>
    inline void Cell::VisitGridObjects(const Occupant* center_obj, T& visitor, float radius, bool dont_load)
{
...
template<class T>
    inline void Cell::VisitWorldObjects(const Occupant* center_obj, T& visitor, float radius, bool dont_load)
{
    CellPair p(MaNGOS::ComputeCellPair(center_obj->Where().X(), center_obj->Where().Y()));
    Cell cell(p);
    if (dont_load)
...
template<class T>
    inline void Cell::VisitAllObjects(const Occupant* center_obj, T& visitor, float radius, bool dont_load)
{
    CellPair p(MaNGOS::ComputeCellPair(center_obj->Where().X(), center_obj->Where().Y()));
    Cell cell(p);
    if (dont_load)
