...
bool CharacterRows::WriteCharacterList(QueryResult* result, WorldPacket* p_data)
{
    ...
    *p_data << MakeGuid(HIGHGUID_PLAYER, guid);
    *p_data << fields[1].GetString();
    *p_data << uint8(pRace);
    *p_data << uint8(pClass);
    *p_data << uint8(fields[4].GetUInt8());

    uint32 playerBytes = fields[5].GetUInt32();
    *p_data << uint8(playerBytes);
    *p_data << uint8(playerBytes >> 8);
    *p_data << uint8(playerBytes >> 16);
    *p_data << uint8(playerBytes >> 24);

    uint32 playerBytes2 = fields[6].GetUInt32();
    *p_data << uint8(playerBytes2 & 0xFF);

    *p_data << uint8(fields[7].GetUInt8());
    *p_data << uint32(fields[8].GetUInt32());
    *p_data << uint32(fields[9].GetUInt32());

    *p_data << fields[10].GetFloat();
    *p_data << fields[11].GetFloat();
    *p_data << fields[12].GetFloat();

    *p_data << uint32(fields[13].GetUInt32());
...
void CharacterRows::Delete(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    uint32 lowguid = GuidCounter(playerguid);
        ...
        case 0:
        {
            ...
            if (resultMail)
            {
                do
                {
                    ...
                    if (mailTemplateId)
                    {
                        draft.SetMailTemplate(mailTemplateId, false);
                    }
                    else
                    {
                    ...
                    draft.SetMoney(money).SendReturnToSender(pl_account, playerguid, MakeGuid(HIGHGUID_PLAYER, sender));
                }
            ...
            if (resultFriend)
            {
                do
                {
                    ...
                    if (Player* sFriend = sPlayerRegistry.Find(MakeGuid(HIGHGUID_PLAYER, fieldsFriend[0].GetUInt32())))
                    {
...
void CharacterRows::DeleteLongDeleted(uint32 keepDays)
{
    ...
    if (resultChars)
    {
        ...
        do
        {
            ...
            ObjectGuid guid = MakeGuid(HIGHGUID_PLAYER, charFields[0].GetUInt32());
...
{
    QueryResult* result = CharacterDatabase.PQuery("SELECT `rank` FROM `guild_member` WHERE `guid`='%u'", GuidCounter(guid));
    if (result)
    {
...
{
    QueryResult* result = CharacterDatabase.PQuery("SELECT `position_x`,`position_y`,`position_z`,`orientation`,`map`,`taxi_path` FROM `characters` WHERE `guid` = '%u'", GuidCounter(guid));
    if (!result)
    {
        return false;
    }
...
void CharacterRows::SetPlaceOf(ObjectGuid guid, uint32 mapid, float x, float y, float z, float o, uint32 zone)
{
    ...
    ss << "UPDATE `characters` SET `position_x`='" << x << "',`position_y`='" << y
       ...
       << "`transguid`='0',`taxi_path`='' WHERE `guid`='" << GuidCounter(guid) << "'";
