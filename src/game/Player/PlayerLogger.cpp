...
PlayerLogger::PlayerLogger(ObjectGuid const & guid) : logActiveMask(0), playerGuid(GuidCounter(guid))
{
...
void PlayerLogger::LogDamage(bool done, uint16 damage, uint16 heal, ObjectGuid const & unitGuid, uint16 spell)
{
    ...
    log.dmgUnit = (GuidCounter(unitGuid) == playerGuid) ? 0 : ((unitGuid != 0 && GuidHigh(unitGuid) == HIGHGUID_PLAYER) ? GuidCounter(unitGuid) : GuidEntry(unitGuid));
    log.SetCreature((GuidHigh(unitGuid) == HIGHGUID_UNIT || GuidHigh(unitGuid) == HIGHGUID_PET));
...
void PlayerLogger::LogLooting(LootSourceType type, ObjectGuid const & droppedBy, ObjectGuid const & itemGuid, uint32 id)
{
    ...
    log.itemEntry = GuidEntry(itemGuid);
    log.SetLootSourceType(type);
    log.itemGuid = GuidCounter(itemGuid);
    log.droppedBy = (droppedBy == 0) ? id : GuidEntry(droppedBy);
...
void PlayerLogger::LogTrading(bool aquire, ObjectGuid const & partner, ObjectGuid const & itemGuid)
{
    ...
    log.itemGuid = GuidCounter(itemGuid);
    log.partner = GuidCounter(partner);
...
void PlayerLogger::LogKilling(bool killedEnemy, ObjectGuid const & unitGuid)
{
    ...
    log.unitEntry = GuidEntry(unitGuid);
    log.SetKill(killedEnemy);
    log.unitGuid = GuidCounter(unitGuid);
...
{
    Player* pl = sPlayerRegistry.Find(MakeGuid(HIGHGUID_PLAYER, playerGuid), true);
    if (!pl)
    {
        pl = sPlayerRegistry.Find(MakeGuid(HIGHGUID_CORPSE, playerGuid), true);
    }
