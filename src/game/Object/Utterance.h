 ...
 */
void Utter(WorldObject const& speaker, ChatType kind, char const* text,
           Unit const* target = nullptr, Language language = LANG_UNIVERSAL);

void Utter(WorldObject const& speaker, MangosStringLocale const* line, Unit const* target = nullptr);
...
void PlaySound(WorldObject const& source, SoundKind kind, uint32 soundId, Player const* target = nullptr);
