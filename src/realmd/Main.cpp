...
extern int main(int argc, char** argv)
{
    ...
    char const* cfg_file = MangosVersion::RealmdConfigLocation();
    ...
    if (!sConfig.SetSource(cfg_file))
    {
        ...
        if (!sConfig.SetSource(MangosVersion::RealmdConfigName()))
        {
            sLog.outError("Could not find configuration file %s.", cfg_file);
            Log::WaitBeforeContinueIfNeed();
            return 1;
        }
        cfg_file = MangosVersion::RealmdConfigName();
    }
    ...
    sLog.outString("%s [realm-daemon]", MangosVersion::ProductRevision());
    ...
    if (confVersion < MangosVersion::RealmdConfigVersion())
    {
        sLog.outError("*****************************************************************************");
